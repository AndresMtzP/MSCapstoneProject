...
class Node(object):
    ...
    def __init__(self, data=None, left=None, right=None, highdimnode=None):
        self.data = data
        self.left = left
        self.right = right
        self.highdimnode = highdimnode
...
class KDNode(Node):
    ...
    def __init__(self, data=None, left=None, right=None, axis=None,
            sel_axis=None, dimensions=None, highdimnode=None):
        ...
        node. """
        super(KDNode, self).__init__(data, left, right, highdimnode)
        self.axis = axis
    ...
    def fast_search(self, point):
        this_dist = self.dist(point)

        if point[self.axis] <= self.data[self.axis]:
            if self.left is None or self.left.data is None:
                return (self, this_dist)
            node, dist = self.left.fast_search(point)
        else:
            if self.right is None or self.right.data is None:
                return (self, this_dist)
            node, dist = self.right.fast_search(point)
...
def create(point_list=None, dimensions=None, axis=0, sel_axis=None):
    ...
    if axis != dimensions-1:
        rand_ax = list(range(0, axis)) + list(range(axis+1, dimensions))
    else:
        rand_ax = range(0, axis)
    sel_axis  = random.choice(rand_ax)
    ...
    loc   = point_list[median][0]
    highdimloc = point_list[median][1]
    left  = create(point_list[:median], dimensions, sel_axis)
    right = create(point_list[median + 1:], dimensions, sel_axis)
    return KDNode(loc, left, right, axis=axis, sel_axis=sel_axis, dimensions=dimensions, highdimnode=highdimloc)
